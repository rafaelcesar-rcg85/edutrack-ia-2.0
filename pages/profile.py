    ...
    with st.form('profile_form'):
        ...
        if st.form_submit_button('💾 Salvar Perfil', use_container_width=True):
            if not first_name.strip():
                ...
                st.error("O campo 'Sobrenome' é obrigatório.")
            else:
                payload = {
    ...
    with st.expander('Clique aqui para alterar sua senha', expanded=False):
        with st.form('change_password_form'):
            ...
            if submitted:
                ...
                elif len(new_password) < 8 or not any(c.isalpha() for c in new_password):
                    st.error('A nova senha deve ter pelo menos 8 caracteres e conter pelo menos uma letra.')
